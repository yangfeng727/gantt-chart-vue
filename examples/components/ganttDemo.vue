...
<template>
    <div class="wrap">
        <div class="header" v-if="showOther">
            <span>显示模式：</span>
            <el-select v-model="showMode" placeholder="请选择" @change="filterGanTTTags">
...
export default {
    ...
    methods: {
        ...
        logParams() {
            ...
            console.log('ganTT1 标记线贯穿tags', ganTT1.getTimePierceTags(this.markLineTime))
            console.log('ganTT2 标记线贯穿tags', ganTT2.getTimePierceTags(this.markLineTime))
        ...
        tagMenuBtnClick2(data) {
            ...
            if (target.value === 'addPreIcon') { // tag前添加图标
                this.$refs['ganTT2'].updateTag(tag.tagId, {
                    ...tag,
                    preIcon: !tag.preIcon ? 'el-icon-goods' : ''
                }, false)
            }

            if (target.value === 'merge') { // 合并
                if (this.selectTagIds.length < 2) return this.$message({
                    message: '请选择需要合并的两项',
                    type: 'warning'
                })
        ...
        tagClick2(data) {
            ...
            if (+tagItem.type !== 1) return this.$message({
                message: '只有第一行可以选中',
                type: 'warning'
            });
            ...
            if (this.selectTagIds.length) { // 之前有选中项
                let firstSelectedTagId = this.selectTagIds[0] // 第一个id
                let firstSelectedTag = selectedTagsInGanTT.find(_item => _item.tagId === firstSelectedTagId)
                if (!firstSelectedTag) { // 没有匹配项，说明甘特图中的tagid 重新生成了了 -- 之前的id无意义，建议给每个tag来个唯一id这样就会避免这种情况【如这个demo，撤回后这部分选中状态将消失】
                    // 这里将丢失之前的选中顺序
                ...
                if (recordRowLabel !== tagItem.parentKey) return this.$message({
                    message: '请选择相同行',
                    type: 'warning'
                });
                ...
                if (!isInRecord && _selected && this.selectTagIds.length >= 2) {
                    ...
                    this.$refs['ganTT2'].updateTag(firstSelectedTagId, {
                        ...firstSelectedTag,
                        selected: false
                    }, false)
                }
            ...
            this.$refs['ganTT2'].updateTag(tagId, {
                ...tagItem,
                selected: _selected
            }, false)
        ...
        tagContextmenuHandle2({ tag }) {
            let findLgend = this.ganTT2Option.legend.find(item => item.type === tag.type)
            let findLegendItem = findLgend.btnList.find(item => item.value === 'addPreIcon')
